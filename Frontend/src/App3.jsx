...
const WelcomeMessage = () => (
  <div className="text-center p-4">
    <h2 className="text-2xl font-bold text-gray-800">Welcome to your Interview Practice Session</h2>
...
// (Header now has the round selection in the center, and the Listening indicator is fixed at the top right of the chat window.)
//
// Note: The chat container has a "relative" class so that an absolutely positioned indicator with a high z-index (and pointer-events-none)
// will always remain visible.
const InterviewChat = ({ token, username, onLogout }) => {
  const [listening, setListening] = useState(false);
  // Maintain separate messages for each round.
  const [messages, setMessages] = useState({ technical: [], hr: [] });
  const [currentRound, setCurrentRound] = useState("technical"); // default round is technical
  ...
  const handleSpeechResult = async (finalText) => {
    // Append the user message for the current round.
    setMessages((prev) => ({
      ...prev,
      [currentRound]: [
        ...prev[currentRound],
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/query", {
        ...
        body: JSON.stringify({ text: finalText, round: currentRound }),
      });
      const data = await res.json();
      const newMsg = { sender: "assistant", text: data.response, timestamp: new Date().toLocaleTimeString() };
      setMessages((prev) => ({
        ...prev,
        [currentRound]: [...prev[currentRound], newMsg],
      }));
      ...
      const msgIndex = messages[currentRound].length + 1; // using current round's message count
  ...
  return (
    <div className="min-h-screen bg-[#A9B5DF] flex flex-col">
      {/* Header with Title, Centered Round Selection, and Right Side Logout */}
      <header className="bg-white shadow-xl rounded-lg p-4 flex items-center justify-between">
        <div className="flex items-center">
          <h1 className="text-3xl font-bold text-gray-800">Interview Mentor</h1>
        </div>
        <div className="flex space-x-4">
          <button
            className={`px-4 py-2 rounded ${currentRound === "technical" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-800"}`}
            onClick={() => setCurrentRound("technical")}
          >
            Technical
          </button>
          <button
            className={`px-4 py-2 rounded ${currentRound === "hr" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-800"}`}
            onClick={() => setCurrentRound("hr")}
          >
            HR
          </button>
        </div>
      ...
      {/* Chat Window */}
      <div className="w-1/2 h-screen mx-auto p-4 flex flex-col">
        {/* Chat container with custom scrollbar styling */}
        <div ref={chatboxRef} className="relative bg-gray-50 border border-gray-200 rounded-lg p-4 flex-1 overflow-y-auto mb-4 custom-scrollbar">
          {/* Always-visible Listening indicator at top right with high z-index */}
          <div className="absolute top-2 right-2 z-50 pointer-events-none px-2 py-1 bg-green-500 text-white rounded text-sm">
            Listening
          </div>
          {messages[currentRound] && messages[currentRound].length > 0 ? (
            messages[currentRound].map((msg, index) => (
